...
def main(cap):
    ...
    if results.pose_world_landmarks:
        ...
        #print( "RecognitionSquatDown\nleft_knee: ", angle_left_knee, "right_knee: ", angle_right_knee )        
        if (angle_left_knee <= 180 and angle_left_knee >= 150 and angle_right_knee >= 90 and angle_right_knee <= 120) or \
           (angle_right_knee <= 180 and angle_right_knee >= 150 and angle_left_knee >= 90 and angle_left_knee <= 120): # 綠色 標準動作
            #cv2.putText(preview, "Left Angle: {:f}".format(angle_left_knee), (1400, 920), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1, cv2.LINE_AA)
            return 1
              
        elif (angle_left_knee <= 150 and angle_left_knee >= 120 and angle_right_knee > 120 and angle_right_knee <= 150) or \
             (angle_right_knee <= 150 and angle_right_knee >= 120 and angle_left_knee > 120 and angle_left_knee <= 150): # 黃色
