...
def CompositePicture(input_head, input_body, i):
  if os.path.isfile(input_head) and os.path.isfile(path_input + input_body):
    ...
    resultPicture.save( path_output + "/RUN_" + str(i) + ".png" )
...
def main():
  ...
  RunArr = []
  for i in range(16):
    RunArr.append('/player'+ str(i+1) + '.png' )

  for i in range(16):
    if ( not CompositePicture(head,  RunArr[i], i+1 ) ) :
       return False
  return True 
